...
def inv_cov_fft_vgg(alpha, beta, gamma, kappa, H, W):
...
def log_det_fft(alpha, beta, gamma, kappa, H, W):
    result = inv_cov_fft_vgg(alpha, beta, gamma, kappa, H, W)
    return result.log().sum()

def fft_Lambda_y(alpha, beta, gamma, kappa, H, W, Y, power=1):
    Y_fft = torch.rfft(Y[0], 3, onesided=False)
    A_fft = inv_cov_fft_vgg(alpha, beta, gamma, kappa, H, W)[:,:,:,None]
    return torch.irfft(A_fft**power * Y_fft, 3, onesided=False) 

def conv_obj(alpha, beta, gamma, kappa, Y, H, W):
    m,c,H,W = Y.shape
...
def f_objective(alpha, beta, gamma, kappa, Y, H, W):
    return  conv_obj(alpha, beta, gamma, kappa, Y, H, W) - log_det_fft(alpha, beta, gamma, kappa, H, W)
...
def newton(X, tol=1e-4, max_iter=500):
    m,c,H,W = X.shape
    ...
    kappa = torch.tensor(-0.75, requires_grad=True)
    it = 0
    g = torch.tensor(1.)
    while (g.norm().item() > tol and it < max_iter):
        f = f_objective(alpha, beta, gamma, kappa, X, H, W)
        g, H_ = gradient_hessian(f, [alpha, beta, gamma, kappa])
        ...
        while True:
            ...
            f0 = f_objective(alpha0, beta0, gamma0, kappa0, X, H, W)
            if f0 < f+1000:
                break
            t = t*0.5
        ...
        it += 1
        print(alpha.item(), beta.item(), gamma.item(),kappa.item())
    return alpha, beta, gamma, kappa

def inference_vgg(IMAGENET_PATH):
    ...
    G = G/num
    alpha, beta, gamma, kappa = newton(G)
    return alpha.item(), beta.item(), gamma.item(), kappa.item()